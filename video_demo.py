# -*- coding: utf-8 -*-
"""
Created on Sat Jul 20 23:24:19 2019
...
A = np.zeros((300,1))
start = 100
for n in range(start,100000,1):
    s = time.time()    
    frame = np.load('test_video/'+str(n)+'.npy')  
    ...
    if len(Pose)<15:
        ...
        pose_result = get_mode(Pose[(n-start)-14:(n-start)+1])
...
#            pose_result = pose[int(Action[-1])]                                                                                                                       
    ...
    result = cv2ImgAddText(result, str(n-start), 3, 0, (255, 255, 255), 60)                   
